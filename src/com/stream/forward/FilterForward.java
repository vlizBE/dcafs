...
public class FilterForward extends AbstractForward {
    ...
    public boolean writeToXML( XMLfab fab ){
        return writeToXML(fab,true);
    }
    public boolean writeToXML( XMLfab fab, boolean allowShort ){
        xml = fab.getXMLPath();
        xmlOk=true;
        ...
        if( allowShort && sources.size()==1 && rules.size()<=1){
            fab.attr("src",sources.get(0));
            if( rules.size()==1){
                fab.attr("type",rulesString.get(0)[1]).content(rulesString.get(0)[2]);
            }
        }else{
            fab.alterChild("sources").down()
                    .comment("Sources go here");
            sources.forEach( src -> fab.addChild("source", src) );

            fab.up()
                    .alterChild("rules").down().comment("Rules go here, possible types: start,nostart,end,contains,c_start,c_end");
    ...
    public void addStartsWith( String with ){
        rulesString.add( new String[]{"",with,"start"} );
        rules.add( p -> p.startsWith(with) );
    }
    public void addStartsNotWith( String with ){
        rulesString.add( new String[]{"",with,"nostart"} );
        rules.add( p -> !p.startsWith(with) );
    }
